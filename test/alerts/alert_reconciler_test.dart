...
void main() {
  ...
  group('opening new alerts', () {
    test('SOC below warning threshold with no existing alert opens a warning', () {
    ...
    test('SOC below critical threshold opens critical directly, not warning first', () {
      final mutations = AlertReconciler.reconcile(
    ...
    test('battery temp above threshold opens an overheat alert independent of SOC', () {
      final mutations = AlertReconciler.reconcile(
      ...
      expect(mutations, hasLength(1));
      expect((mutations.single as OpenNewAlert).type, AlertType.batteryOverheat);
    });
    ...
    test('no duplicate alert opened when one is already open for the same vehicle+type', () {
      final mutations = AlertReconciler.reconcile(
  ...
  group('escalation and de-escalation while active', () {
    test('active warning escalates to critical when SOC drops further', () {
      final mutations = AlertReconciler.reconcile(
        readings: [reading(soc: 5)],
        openAlerts: [open(id: 7, severity: AlertSeverity.warning, status: AlertStatus.active)],
        now: now,
      );
    ...
    test('active critical de-escalates to warning when SOC recovers a bit', () {
      final mutations = AlertReconciler.reconcile(
        readings: [reading(soc: 15)],
        openAlerts: [open(id: 7, severity: AlertSeverity.critical, status: AlertStatus.active)],
        now: now,
      );
  ...
  group('dismissed alerts', () {
    test('escalation reactivates a dismissed warning as critical', () {
      final mutations = AlertReconciler.reconcile(
        readings: [reading(soc: 5)],
        openAlerts: [open(id: 9, severity: AlertSeverity.warning, status: AlertStatus.dismissed)],
        now: now,
      );
    ...
    test('de-escalation while dismissed produces no mutation (stays dismissed)', () {
      final mutations = AlertReconciler.reconcile(
        readings: [reading(soc: 15)],
        openAlerts: [open(id: 9, severity: AlertSeverity.critical, status: AlertStatus.dismissed)],
        now: now,
      );
  ...
  group('resolution', () {
    ...
    test('condition clearing resolves a DISMISSED alert too — independent of dismissal', () {
      final mutations = AlertReconciler.reconcile(
        readings: [reading(soc: 90)],
    ...
    test('resolved alert re-triggering opens a brand new alert, not a revival', () {
  ...
  group('staleness freezes evaluation', () {
    test('stale reading with no existing alert opens nothing, even if SOC is critical', () {
      final mutations = AlertReconciler.reconcile(
        readings: [reading(soc: 2, lastSeen: now.subtract(const Duration(minutes: 30)))],
        openAlerts: [],
        now: now,
      );
    ...
    test('stale reading does NOT resolve an existing alert even though soc is now null-ish/healthy', () {
      final mutations = AlertReconciler.reconcile(
        readings: [reading(soc: 90, lastSeen: now.subtract(const Duration(minutes: 30)))],
        openAlerts: [open(id: 3, status: AlertStatus.active)],
        now: now,
      );
    ...
    test('reading exactly at the staleness boundary is still fresh (uses > not >=)', () {
      final mutations = AlertReconciler.reconcile(
        readings: [
  ...
  group('multiple vehicles and types are independent', () {
    ...
    test('SOC alert and overheat alert on the same vehicle are tracked separately', () {
      final mutations = AlertReconciler.reconcile(
